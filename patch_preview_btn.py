...
target = """            Text("Unsynced Files: ${unsyncedFiles.size}", modifier = Modifier.padding(start = 12.dp, top = 8.dp, bottom = 4.dp))
            
            val filteredFiles = files.filter { it.path.contains(searchQuery, ignoreCase = true) }.sortedBy { it.path }"""

replacement = """            Text("Unsynced Files: ${unsyncedFiles.size}", modifier = Modifier.padding(start = 12.dp, top = 8.dp, bottom = 4.dp))
...
if target in content:
    ...
    with open('app/src/main/java/com/example/service/AppyworkWindowManager.kt', 'w') as f:
        f.write(content)
    print("Patched preview btn successfully")
else:
    print("Target preview btn not found")
