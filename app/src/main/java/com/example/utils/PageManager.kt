...
data class SidebarPage(
    ...
    companion object {
        fun createDefault(id: String, type: String, title: String): SidebarPage {
            val wrap = when(type) { "calculator", "compass", "notification", "scheduler", "reader", "widget" -> false else -> true }
            val h = when(type) { "calculator" -> 450; "compass" -> 500; "notification", "scheduler", "reader", "widget" -> 500; else -> 450 }
            return SidebarPage(
