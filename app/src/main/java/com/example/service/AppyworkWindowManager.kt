...
import com.example.AppyworkSettingsActivity
...
class AppyworkWindowManager(
    private val context: Context,
    private val parsedBlocks: List<ParsedCodeBlock>,
    private val onClose: () -> Unit
) {
    private val windowManager = context.getSystemService(Context.WINDOW_SERVICE) as WindowManager
    private var floatingView: View? = null
    
    private val prefs = context.getSharedPreferences("AppyworkPrefs", Context.MODE_PRIVATE)
    ...
    fun show() {
        ...
        floatingView = inflater.inflate(R.layout.layout_appywork_floating, null)

        val expandedContainer = floatingView?.findViewById<LinearLayout>(R.id.appywork_expanded_container)
        ...
        val btnClose = floatingView?.findViewById<Button>(R.id.appywork_btn_close)
        val btnFold = floatingView?.findViewById<Button>(R.id.appywork_btn_fold)
        val btnResize = floatingView?.findViewById<ImageView>(R.id.appywork_btn_resize)
        val btnSettings = floatingView?.findViewById<ImageView>(R.id.appywork_btn_settings)
        
        val contentContainer = floatingView?.findViewById<FrameLayout>(R.id.appywork_content_container)
        ...
        ActiveAppTracker.addApp("appywork_${this.hashCode()}", "Appywork", "Code", 60)
        ...
        btnFold?.setOnClickListener {
            toggleMinimize(expandedContainer, bubble, density)
        }
        ...
        btnSettings?.setOnClickListener {
            val intent = Intent(context, AppyworkSettingsActivity::class.java).apply {
    ...
    private fun applyBlocksToProject(project: AppyworkProject) {
    ...
    fun close() {
        ActiveAppTracker.removeApp("appywork_${this.hashCode()}")
        try {
...
fun AppyworkProjectSelector(parsedBlocks: List<ParsedCodeBlock>, onProjectSelected: (AppyworkProject) -> Unit) {
    ...
    var currentBlocks by androidx.compose.runtime.remember(parsedBlocks) { androidx.compose.runtime.mutableStateOf(parsedBlocks) }
    ...
    if (selectedProject == null) {
        ...
        Column(modifier = Modifier.fillMaxSize()) {
            ...
            val filteredFiles = files.filter { it.path.contains(searchQuery, ignoreCase = true) && it.syncState != "DELETED" }.sortedBy { it.path }
